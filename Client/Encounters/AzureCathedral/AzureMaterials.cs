...
internal static class AzureMaterials
{
    ...
    internal static void Worm(AzureWorm head, AzureBoss girl, SpriteBatch batch, Vector2 screen)
    {
        ...
        float age = AzureVisuals.RenderAge(girl), presence = AzureRules.Ease((age - girl.State.MusicStart - 140) / 160);
        ...
        var shader = Begin(); shader.SetTexture(art, 0, SamplerState.LinearClamp); shader.TrySetParameter("clock", age / 60);
        for (int i = AzureRules.Segments; i >= 0; i--)
        {
            ...
            float angle = part.NPC.rotation, scale = i == 0 ? 258 : i == AzureRules.Segments ? 190 : 163;
            if (i == 0)
            ...
            float flex = AzureVisuals.Reduced ? 0 : MathF.Sin(age * .038f - i * .38f) * .045f;
            shader.TrySetParameter("region", uv);
            shader.TrySetParameter("signal", new Vector4(presence, head.NPC.ai[2], AzureVisuals.Reduced ? 1 : 0, i));
