...
internal sealed class FirstSeveranceEmissionVisuals
{
    ...
    internal static Color AttackColor(FirstSeveranceLanceVolley v)
        => v.Kind == FirstSeveranceAttackKind.PursuitPrism
            ? FirstSeveranceAttackPatterns.PrismColorIndex(v.Step) switch
    ...
    internal void Draw(SpriteBatch batch, ulong authorityTick, bool reduced)
    {
        ...
        foreach (Emitter e in emitters)
        {
            ...
            for (int r = 0; r < v.Rays.Count; r++)
            {
                ...
                {
                    float t = Frac((float)(now - v.StartTick) * .023f + knot * .217f);
                ...
                float age = (float)(now - v.FireTick);
                float shock = Window(now, v.FireTick - 2d, v.FireTick + 3d) * (1 - Window(now, v.FireTick + 3d, v.EndTick + 12d));
                if (shock > 0)
    ...
    private void DrawAperture(SpriteBatch batch, Vector2 mouth, Vector2 direction, double tick,
        FirstSeveranceLanceVolley v, float open, float emission, Color color, bool reduced)
    {
        float angle = MathF.Atan2(direction.Y, direction.X);
        Vector2 normal = new(-direction.Y, direction.X);
        float radius = 15 + open * 58;
        float imminent = PreRelease(tick, v.FireTick, Math.Min(16, v.TelegraphTicks));
        Accents.CastSeal(batch, mouth, tick, v.StartTick, v.FireTick, color, reduced, .75f);
        ...
        float gathering = open * (1 - Window(tick, v.FireTick, v.FireTick + 10d));
        for (int f = 0; f < (reduced ? 3 : 8); f++)
        {
            ...
            {
                float t = s / 24f;
                float phase = f * MathHelper.TwoPi / 8 + (float)(tick - v.StartTick) * (.04f + open * .10f) + t * 3;
