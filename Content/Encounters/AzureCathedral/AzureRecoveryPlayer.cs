...
public sealed class AzureRecoveryPlayer : ModPlayer
{
    private static readonly HashSet<AzureRecoveryPlayer> bound = new();
    private Guid fight, terminalFight;
    private AzureRecoveryState health;
    private AzureDownLatch latch;
    private AzureNativeFloor floor;
    ...
    internal static void Apply(AzureState state)
    {
        foreach (var m in state.Members ?? Array.Empty<AzureMember>())
        {
            var p = Main.player[m.Slot];
            var adapter = p.GetModPlayer<AzureRecoveryPlayer>();
            if (m.Out || !p.active || p.dead || p.ghost
                || Main.netMode != NetmodeID.MultiplayerClient && p.GetModPlayer<AzureConnection>().Token != m.Connection)
            { if (adapter.fight == state.Fight) adapter.Clear(); continue; }
            if (state.Stage == AzureStage.Defeat)
            {
                ...
                if (newDefeat && adapter.OwnsHurt)
                {
                    p.KillMe(PlayerDeathReason.ByCustomReason(NetworkText.FromKey(
                        "Mods.Convergence.AzureRecovery.DefeatReason", p.name)), 9999, 0);
                    if (!p.dead) AzurePackets.Log($"event=DefeatDeathBlocked fight={state.Fight} slot={p.whoAmI} external_pre_kill=True");
                }
                continue;
            }
            if (state.Stage == AzureStage.Victory)
            { if (adapter.fight == state.Fight) adapter.Clear(); continue; }
            if (m.Recovery.Revision == 0 || state.Stage < AzureStage.Countdown || adapter.terminalFight == state.Fight) continue;
            adapter.ApplyState(state.Fight, m.Recovery, state.Age);
        }
    }
    private void ApplyState(Guid owner, AzureRecoveryState next, int age)
    {
        ...
        downBuff = ModContent.BuffType<AzureDownedDebuff>();
    ...
    public override void PreUpdate()
    {
        if (fight != Guid.Empty && (Main.gameMenu || !Player.active || Player.dead || Player.ghost
            || Main.GameUpdateCount > validUntil || AzurePackets.Snapshot.FightId.Value != fight)) Clear();
    }
    ...
    private void LatchFloor(string source)
    {
        if (!IsIncapacitated)
        {
            ...
            AzurePackets.Log($"event=NativeDownFloor fight={fight} slot={Player.whoAmI} health_revision={health.Revision} source={source}");
        }
        Player.statLife = 1;
        if (!health.Downed && Main.GameUpdateCount >= nextFloorReport)
        {
            nextFloorReport = Main.GameUpdateCount + 30;
            AzurePackets.RequestRecovery(revive: false);
        }
    }
...
public sealed class AzureDownedDebuff : ModBuff
{
...
internal sealed class AzureRecoverySystem : ModSystem
{
    public override void OnWorldUnload() => AzureRecoveryPlayer.ClearAll();
    public override void Unload() => AzureRecoveryPlayer.ClearAll();
}
