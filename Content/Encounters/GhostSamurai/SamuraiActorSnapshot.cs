...
internal readonly record struct SamuraiActorSnapshot(Guid Fight, int Age, SamuraiPhase Phase, SamuraiAttack Attack,
    SamuraiBeat Beat, int AttackTimer, int TransitionRemaining, int MaximumLife)
{
    internal bool IsValid => Fight != Guid.Empty && Age >= 0 && Enum.IsDefined(Phase) && Enum.IsDefined(Attack)
    ...
    internal static SamuraiActorSnapshot Read(BinaryReader reader)
    {
        ...
        var result = new SamuraiActorSnapshot(new Guid(bytes), age, phase, attack, beat, timer, transition, life);
