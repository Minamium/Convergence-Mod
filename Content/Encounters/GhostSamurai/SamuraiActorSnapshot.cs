...
internal readonly record struct SamuraiActorSnapshot(Guid Fight, int Age, SamuraiPhase Phase, SamuraiAttack Attack,
    SamuraiBeat Beat, int AttackTimer, int TransitionRemaining, int MaximumLife, SamuraiArenaBounds Arena, int LockedTarget = -1)
{
    internal bool IsValid => Fight != Guid.Empty && Arena.IsValid && LockedTarget is >= -1 and < 255 && Age >= 0 && Enum.IsDefined(Phase) && Enum.IsDefined(Attack)
    ...
    internal void Write(BinaryWriter writer)
    {
        ...
        Arena.Write(writer); writer.Write((short)LockedTarget);
    }
    internal static SamuraiActorSnapshot Read(BinaryReader reader)
    {
        ...
        var result = new SamuraiActorSnapshot(new Guid(bytes), age, phase, attack, beat, timer, transition, life, SamuraiArenaBounds.Read(reader), reader.ReadInt16());
