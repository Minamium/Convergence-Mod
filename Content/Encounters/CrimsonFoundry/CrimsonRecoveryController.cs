...
internal enum CrimsonRecoveryReply : byte { None, Revived, NoTarget, Locked, RequiresKit, Invalid }
...
internal sealed class CrimsonRecoveryController
{
    ...
    private readonly CrimsonRecoveryState[] health;
    private readonly CrimsonNativeFloor[] floors;
    private readonly uint[] downNonces, reviveNonces;
    private readonly bool[] pendingDowns;
    private readonly CrimsonRecoveryRequest?[] pendingRevives;
    internal bool Failed => service.FailureReason != RaidReviveFailureReason.None;

    internal CrimsonRecoveryController(Guid id, CrimsonMember[] roster)
    {
        fight = new(id); bindings = new RaidParticipantBinding[roster.Length];
        connections = new Guid[roster.Length]; health = new CrimsonRecoveryState[roster.Length];
        floors = new CrimsonNativeFloor[roster.Length];
        downNonces = new uint[roster.Length]; reviveNonces = new uint[roster.Length];
        pendingDowns = new bool[roster.Length];
        pendingRevives = new CrimsonRecoveryRequest?[roster.Length];
        for (int i = 0; i < roster.Length; i++)
        {
            ...
            if (!p.HasItem(kit)) p.QuickSpawnItem(new EntitySource_Misc("Convergence.CrimsonRecovery"), kit);
        }
        service = new(fight, RaidReviveSettings.CreateInstantUnlimited(roster.Length), bindings);
    }

    private int Index(int sender, in CrimsonRecoveryRequest request)
    {
    ...
    private bool Connected(int i)
    {
        ...
        return p.active && !p.dead && !p.ghost && p.GetModPlayer<CrimsonConnection>().Token == connections[i];
    }
    internal bool Receive(int sender, CrimsonRecoveryRequest request, bool revive)
    {
    ...
    internal void Project(CrimsonMember[] members)
    { for (int i = 0; i < health.Length; i++) members[i] = members[i] with { Recovery = health[i] }; }

    internal bool Tick(CrimsonState state, CrimsonMember[] members)
    {
        ...
        for (int i = 0; i < bindings.Length; i++)
        {
            ...
            if (!health[i].Downed && nativeFloor)
            {
                ...
                {
                    var anchor = CrimsonChoreography.ClampParticipant(state.Field, player.position.X, player.position.Y, player.width, player.height);
                    ...
                    CrimsonPackets.Log($"event=Downed fight={fight.Value} age={state.Age} slot={player.whoAmI} health_revision={health[i].Revision} anchor={anchor.X:F1},{anchor.Y:F1} basis=NativeHpFloor");
                }
            }
        }
        ...
        for (int i = 0; i < bindings.Length; i++)
        {
            ...
            if (reply != CrimsonRecoveryReply.None) { CrimsonPackets.RecoveryReply(p.whoAmI, reply); continue; }
        ...
        if (starts.Count > 0)
        {
            ...
            for (int i = 0; i < senders.Count; i++)
            {
                ...
                CrimsonPackets.RecoveryReply(bindings[senders[i]].PlayerSlot,
                    accepted ? CrimsonRecoveryReply.Revived : CrimsonRecoveryReply.Invalid);
            }
        }
        ...
        foreach (var e in commit.Events)
        {
            ...
            CrimsonPackets.Log($"event=Revived fight={fight.Value} age={state.Age} slot={p.whoAmI} by={bindings[e.Actor.Value].PlayerSlot} health_revision={health[i].Revision} life={life} lockout_until={health[i].LockoutUntil}");
            changed = true;
        }
        Project(members);
    ...
    private CrimsonRecoveryReply ValidateRevive(int i, CrimsonRecoveryRequest request, int age, out int target)
    {
        target = -1;
        if (!Connected(i) || health[i].Downed || health[i].Revision != request.HealthRevision) return CrimsonRecoveryReply.Invalid;
        var p = Main.player[bindings[i].PlayerSlot];
        if (p.HeldItem.type != ModContent.ItemType<ResuscitationKitItem>()) return CrimsonRecoveryReply.RequiresKit;
        ...
        return target >= 0 ? CrimsonRecoveryReply.None : locked ? CrimsonRecoveryReply.Locked : CrimsonRecoveryReply.NoTarget;
    }
