...
internal sealed class CrimsonPackets : ModSystem, IEncounterPacketHandler
{
    ...
    internal static CrimsonBoss? Boss
    {
        get
        {
            foreach (NPC n in Main.ActiveNPCs)
                if (n.ModNPC is CrimsonBoss boss && boss.State.Fight == Snapshot.FightId.Value
                    && Snapshot.DefinitionKey == CrimsonDefinition.EncounterKey) return boss;
            return null;
        }
    }
    ...
    public bool TryHandle(BinaryReader reader, int sender, in EncounterPacketHeader header, out string failureCode)
    {
        ...
        else if (header.PacketType == EncounterPacketType.ValidationResult)
        {
            ...
            if (!accepted) Main.NewText(Terraria.Localization.Language.GetTextValue("Mods.Convergence.CrimsonFoundry.Rejected"));
        }
