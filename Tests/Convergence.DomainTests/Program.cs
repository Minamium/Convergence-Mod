...
internal static partial class Program
{
    ...
    private static int Main()
    {
        TestCase[] tests =
        {
            ...
            new("One-player Ready remains manual and one Down ends the Raid on the authority tick", SoloReadyAndDefeat),
            new("Solo debug keeps the two-player workload with one real Stack participant", SoloDebugWorkload),
            new("Terminal HUD cinematics expire and clear without any future packet", TerminalPresentationLifetime),
            new("Development HP scales from the frozen two/three/four-player roster", DevelopmentPartyHp),
            new("Core salvos begin on grid three with bounded immutable directions", CoreSalvoBounds),
            new("Core salvos use the full warning and shared active collision window", CoreSalvoGeometry),
            new("Eclosion pose curves and connected arm joints are continuous", EclosionContinuity),
            new("Boss stages transition at half life without skipping the rupture", BossStageThresholdAndOverkill),
            new("Lattice, spread and blade repeat as one phase score", BossStageWindowAndCap),
            new("Every phase completes its score and zero HP starts the terminal survival score", FullChoreographyAndFinal),
            new("Clockwise sites, wider spreads and practical travel budgets fit the field", ClockAndTravelBudgets),
            new("Sword, hands, half-field, bullets and slicers have harmless warnings and bounded hits", ScoreHazardBoundaries),
            new("Fixed stack anchor is above the Boss and independent of players", FixedStackWorldAnchor),
            new("Lattice warnings, cells and exact hit deadlines share bounded geometry", LatticeGeometryAndTiming),
            new("Stage snapshots reject incompatible or malformed grid state", StageProjectionBounds),
            new("Damage telemetry excludes shielding and clamps deadline/overkill", DamageWindowBoundaries),
            new("Damage telemetry separates recent and whole-window DPS", DamageWindowRates),
            new("Beams use fixed damage while charges and raid shares retain their rules", FixedBeamDamage),
            new("Anticipation is bounded and continuous without loop-reset flashes", SmoothAnticipation),
            new("Emission envelopes remain continuous through firing and cooling", ContinuousEmissionEnvelopes),
            new("Containment is ground anchored and clamps the complete body", GroundContainmentBounds),
            new("Debug assist requires console and opted-in dedicated server", DebugAssistAuthorityGate),
            new("Debug assist permit is single use with exact epoch and deadline", DebugAssistPermitBoundaries),
            new("Debug assist lease is exact Fight/member and cleanup is final", DebugAssistLeaseCleanup),
            ...
            new("Untimed Down survives lockout, remains rescuable and cleans up", UnlimitedRecoveryTimeoutAndCleanup),
            new("Prism targets every standing member with bounded immutable rays", SimultaneousPrismRoster),
            new("Horizontal floods share deployment, widening and moving safe pockets", FloodSafePockets),
            new("Final cadence accelerates and repeated grids have no fixed safe cell", FinalAccelerationAndShifts),
            ...
            new("Fast combo cadence preserves category breathing", FastComboCadencePreservesCategoryBreathing),
            new("Lance snapshot bounds and immutable rays", LanceSnapshotBoundsAndImmutableRays),
            new("Stack uses missing frozen-roster fraction and full gather is free", StackMissingRosterFraction),
            new("Prism locks prediction and reverses color order", PrismPredictionAndOrder),
            new("Dash and stillness corridors match the four-step sequence", DashStillnessSequence),
            new("Attack sequence deadlines and malformed descriptors", AttackSequenceBounds),
            new("Energy charge tracks briefly, locks and cannot tunnel", EnergyChargeMotion),
            new("Only exact-Fight participant Defeat requests normal death", DefeatDeathEligibility),
    ...
    private static void FastComboCadencePreservesCategoryBreathing()
    {
        ...
        AssertEqual(264, FirstSeveranceAttackPatterns.SequenceTicks(FirstSeveranceSubstate.CoreExposure), "four-action sequence budget");
