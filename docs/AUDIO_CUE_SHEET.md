...
`Assets/Sounds/FirstSeverance/Beams/` contains 15 original masters, generated by [generate_beam_sfx.py](../tools/generate_beam_sfx.py). Soft intake → short pre-shot dip → rupture/low pressure → sustained inharmonic foil → bounded release follows the accepted beam material. This is synthesis from event/envelope analysis, **not sampled reference audio or an assertion of subjective listening**. [F16](research/WOTG_RAID_BENCHMARK.md#f16--beam-audio-envelope-analysis-2026-09-13) records source evidence and mixed-recording limitations. User audition is still required.
...
All exports are PCM16 stereo / 44.1 kHz. Asset peak caps are 0.57–0.78; RMS is capped at 0.175 without flattening transient crest factors. Runtime retains the existing 0.8 cue gain and user sound slider, separate from music. Main pursuit allows at most two overlapping voices of each cue. Rotation owns one periodic sustain, faded during its final six live ticks; other timed voices stop by their existing attack deadlines. Offline audition mixes have headroom, but this does not prove full-game output cannot clip or mask music under other Mods.

Recreate runtime assets with `python tools/generate_beam_sfx.py --output Assets/Sounds/FirstSeverance/Beams`. Add `--preview <ignored folder> --ffmpeg <local executable>` for five labeled WAV examples and a combined MP3. Preserve original recordings and generated audition exports outside Git/package; the tracked recipe plus per-name seed is the reproducible master.
