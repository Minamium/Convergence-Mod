...
| Luminance graphics dependency | `Luminance@1.0.14` | Local development target; full compatibility pending | Installed Workshop 1.0.14; shared [presentation policy](ART_DIRECTION.md#luminance-presentation-policy) applies across all content. Native build/offline GPU checks do not establish full compatibility. Reuse the pinned [API evidence](encounters/first-severance/VISUAL_SPEC.md#luminance-raid-presentation) for unchanged questions. |
