...
| Luminance graphics dependency | `Luminance@1.0.14` | Local development target; full compatibility pending | Installed Workshop 1.0.14; Doll Raid materials now use its managed shaders/noise registry. Native build/offline GPU checks do not establish full compatibility. See [visual API boundary](encounters/first-severance/VISUAL_SPEC.md#luminance-raid-presentation). |
