...
Development **0.2.70 / protocol 34** is integrated and installed in the canonical playtest profile: [build evidence](evidence/2026-09-13-flowing-lattice.json). Doll lattice deployment is slower, and each live track carries a finite energy ribbon with a visible head and trailing tail. Collision follows that same tapered packet; sanctuary cuts keep one continuous clock/flow, and embedded Spread follows the third volley's revised timing. [Beam ignition and lattice order](encounters/first-severance/ENCOUNTER_SPEC.md#beam-ignition-and-lattice-order) owns exact rules; [visual spec](encounters/first-severance/VISUAL_SPEC.md#beams-swords-and-forecast-readability) owns the material. Non-lattice attacks, damage values, player circles, audio, scene/ending effects, weapons/rewards and the integrated Ghost Samurai contact-rush/Phase3 work remain unchanged. [build.txt](../build.txt) and [EncounterProtocol](../Common/Networking/Protocol/EncounterProtocol.cs) own version/compatibility. First Severance remains **不幸な人形劇 / The Unfortunate Doll Play**, Boss **ラクリモーサ — 縛られた心 / Lacrimosa — The Bound Heart**. Visual acceptance remains user-owned, not a claim of WoTM/WotG parity.
...
Reload Mods/restart tModLoader to load the installed package; **no additional Build + Reload compilation is needed** for unchanged source. All participants need the matching protocol34 package. User-owned focused acceptance: readable staggered lattice deployment, head/body/tail passage without rectangular switch-off, no damage ahead/behind the visible packet, third-volley Spread pockets, matching peer order including delayed snapshots, Reduced Effects/UI107%/zoom. Luminance is already installed. Do not replay unrelated weapon/loot or stage-ending checks for this narrow follow-up.
