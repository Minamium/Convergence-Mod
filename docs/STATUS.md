...
Development **0.2.66 / protocol 31** integrated into main as `04051a1` (PR #8), then built and installed into the requesting user's normal playtest profile. [build.txt](../build.txt) owns version; [EncounterProtocol](../Common/Networking/Protocol/EncounterProtocol.cs) owns compatibility. The local ModSources entry now resolves to that integrated main checkout, with prior worktrees retained. [Installation evidence](evidence/2026-09-13-ghost-samurai-playtest-install.json) identifies this workstation's package; earlier records describe their own build environments.

First Severance remains **不幸な人形劇 / The Unfortunate Doll Play**, Boss **ラクリモーサ — 縛られた心 / Lacrimosa — The Bound Heart**. Only the main eight-cast PursuitPrism uses the revised Luminance-managed material; the flat fill/sine-wire prototype was rejected and replaced with layered textured convection, hot core, diffuse bloom and a connected pressure plume. [Visual spec](encounters/first-severance/VISUAL_SPEC.md#luminance-prototype-boundary) owns design, scoped opt-in and pinned WoTM continuous-beam research. Its colors, geometry, warning/live/end ticks and sounds are unchanged. Final, Spread's extra pursuit, weapons, other Doll attacks, activation/companion/rewards are retained from integrated main.
...
Ghost Samurai is installed for playtesting on **0.2.66 / protocol31**. Start tModLoader or Reload Mods from the menu, confirm the displayed version, and use the existing bell. All peers need the same protocol31 package. No second unchanged Build + Reload is needed. Test two-hit × four rhythm, immediate movement after the grid ends, late-lock charge/dash dodges, the three-hit Phase2 charge with its longer last hold and additional delayed wisps. Existing summon items remain valid. The user explicitly authorized main integration, normal-profile installation and the corresponding local source alignment; prior worktrees and their uncommitted changes remain preserved.

Reload Mods/restart tModLoader for the installed revised package; another Build + Reload compilation is unnecessary. Confirm the main eight-cast sequence's forecasts → textured plasma release → dim residue, especially full-width readability, Reduced Effects and peer UI107%/zoom. Compare the actual fight before approving wider adoption; the first flat-fill prototype was not accepted. Luminance is already installed locally. Other attacks are deliberately unchanged. Retained activation/companion smoke items are in the [previous evidence](evidence/2026-09-13-doll-beams-and-stage-key.json), not a new mandatory replay queue.
