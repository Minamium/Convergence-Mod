...
Development source: **0.3.15 / protocol46**. Adds five original blue-white/pale-violet slash materials for Ghost Samurai's ordinary, heavy, grid, wind and rush attacks. Warm forecasts, circle inner/outer identity, exact damage volumes and attack clocks are retained. Boss AI, wire layout, Oboro performance, the0.3.14 body-visibility repair and integrated Doll/Crimson work are unchanged. Use matching0.3.15/protocol46 peers when reviewing the new presentation. The separately published baseline remains [0.3.1 / protocol37](releases/0.3.1.md); no new release is implied.
...
- **Ghost Samurai slash materials:** [VFX evidence](evidence/2026-09-17-ghost-samurai-slashes.json) records generated-alpha inspection, linked-production CPU draw containment/clock/cache checks, isolated native compilation and exact-package type checks. Five images render only during the existing live window; no authority/runtime/projectile state changes. Actual SP/MP readability, dark/bright backgrounds, Reduced Effects and frame-time checks remain user-owned / not_run. Normal-profile installation awaits approval of this feature's main integration.
...
The normal profile still has0.3.14 installed; the0.3.15 feature package is isolated pending administrator-approved main integration. After installation, Reload Mods without another Build + Reload. Check the five slash families on bright/dark backgrounds, lattice safe cells, circle safe center, front-cleave safe rear, moving waves and rush body; compare Reduced Effects and a second matching client. The violet body's cold summon/phase-transition visibility, Oboro performance/drop and inherited terrain checks remain separate owner checks, not certified by this VFX change.
