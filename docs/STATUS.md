...
The corrected **0.2.62 / protocol30** package is installed from integrated main. Return to the menu, **enable Convergence again** if the failed load disabled it, Reload Mods, and re-enter the same character/world. Keep the Unloaded Item; tModLoader can restore the unchanged stored Mod/item identity. Verify that the bell returns and can summon the boss. No duplicate Build + Reload is needed. Saves and personal enabled-Mod settings were preserved.
