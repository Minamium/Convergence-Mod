...
Feature-branch source: **0.3.13 / protocol46**, based on integrated0.3.12 (`9f46892`). Branch `feat/scarlet-rhythm-phases` implements Scarlet Invocation's sequential20%-retreat acts, retained-HP four-target Final, authority pursuit/formations, beat-subdivision call/response and fills, and an explicit standard boss-bar adapter. Public display names are Scarlet; stable internal IDs and the accepted recording remain unchanged. Doll, Ghost Samurai and the0.3.12 GPU-lifetime fix are retained. This branch is **not merged, installed or published**; the separately published baseline remains0.3.1/protocol37. Use matching46 peers for this changed schema.
...
- **Scarlet Invocation — revised prototype.** [Owning spec](encounters/crimson-foundry/ENCOUNTER_SPEC.md) covers1–8-player Ready, shared-pedestal combat, sequential20%-retreat apparitions and a four-target Final, musical call/response corridors, moving bodies and native damage. The owner-approved BGM/loop remains included unchanged; intro playback now fades in. Bespoke rewards, extended character motions and calibrated balance remain incomplete.
...
- **0.3.13 Scarlet feature branch:** see [scoped evidence](evidence/2026-09-16-scarlet-rhythm.json). New deterministic cases exercise actual-score phrase timing, loop boundaries, fast fills, collision non-overlap, safe-space overlap,20% floors, four-target victory, party HP budgets and stale phase projections. Static/catalog, compiler and test outcomes are recorded there; **native packaged tModLoader build/load, audible rhythm fit, bar visibility, motion readability and matching-peer playtests are not claimed**. No workstation package was replaced.
...
Review this branch without changing the shared installation. Build an isolated0.3.13/protocol46 package with the pinned tModLoader/Calamity/Luminance dependencies, then confirm load/reload and the new bar/phase/movement behavior. On matching peers, check high-overkill20% transitions, three retained bodies plus performer, wipe-versus-clear, delayed old hazards, distant Ready and repeated entry. Listen to forecast/impact phrasing and compare the actual recording rather than judging musical fit from event counts. Earlier Doll/Ghost Samurai playtest work remains outstanding. No merge, normal-profile install or Workshop/GitHub release is implied.
