...
- **Oboro and violet Ghost Samurai:** [evidence](evidence/2026-09-16-oboro-violet.json) records the isolated0.3.13/protocol46 native build, exact-package loader/codec/regen checks, domain verification and offline light/dark visual inspection. Server/SP owns hits, marks, buff benefits and detonation; client state and visual events are bounded. Runtime-generated cutout textures are created on draw and disposed through the main-thread queue. Equipped Earth comparison, actual drop/save/reload, combat, latency/rejoin and in-game visual checks remain user-owned / not_run. This feature has not yet been installed into the normal profile or approved for main integration.
...
After administrator-approved integration and installation of0.3.13, Reload Mods (re-enable Convergence if the earlier0.3.11 failure disabled it); no repeat Build + Reload is required for an installed package. Confirm Ghost Samurai's violet poses, halo, clear telegraphs and dissolve, then defeat it for Oboro. Check the down/up/heavy three-cut loop, five-second right-click Zanshin, five-wound limit per enemy, manual/expiry detonation and ten-second Wraith Fire. Compare equipped output/range against Earth, and repeat weapon hit/death/disconnect checks with matching peers. Ghost Samurai's inherited bottom-center arrival, overhead blade/downstroke, growing fronts, target loss and cleanup still need its separate owner check; this task does not claim those playtests passed.
