...
The separately merged Ghost Samurai update is retained: two-hit × four rhythm, grid-to-charge movement, late-lock dodges, longer last Phase2 charge hold and delayed wisps. Its original protocol31 payload is retained in the current protocol. Its focused smoke is detailed in the [spec](encounters/ghost-samurai/ENCOUNTER_SPEC.md#検証と試遊); retain the repaired body/line rendering and re-summon cleanup. Retained Doll flight/beam/reward checks live in their evidence; an unrelated full Raid replay is not required.
