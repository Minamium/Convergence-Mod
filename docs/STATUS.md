...
Development **0.2.67 / protocol 31**, integrated native Release package; [build evidence](evidence/2026-09-13-raid-grand-presentation.json) identifies the installed artifact. [build.txt](../build.txt) owns version; [EncounterProtocol](../Common/Networking/Protocol/EncounterProtocol.cs) owns compatibility. First Severance remains **不幸な人形劇 / The Unfortunate Doll Play**, Boss **ラクリモーサ — 縛られた心 / Lacrimosa — The Bound Heart**. The owner authorized a Raid-wide presentation refresh after the eight-cast prototype: all Raid beam paths, energy bullets, source pressure, Stack/Spread verdict accents, stage/sky and both endings now use the new material/staging layers. [Visual spec](encounters/first-severance/VISUAL_SPEC.md#luminance-raid-presentation) owns scope, rendering contracts and pinned research. This Doll pass preserves Raid combat rules/geometry/ticks, sounds/BGM, accepted shell/rig/capture art and weapons/companion/rewards. The separately merged Ghost Samurai changes and protocol31 below are also included, without modifying their implementation. Visual quality is experimental pending playtest, not a claim of WoTM/WotG parity.
...
Reload Mods/restart tModLoader to load the installed package; **no additional Build + Reload compilation is needed** for unchanged source. User-owned focused acceptance is now the refreshed Raid: initial/Spread/Final beam forecasts and actual live widths; lattice/flood safe-space contrast; energy nuclei and crush warning; unchanged player Stack/Spread circles; intro/eclosion and victory/defeat returning cleanly to gameplay. Include the affected peer UI107%/zoom and Reduced Effects checks, not unrelated weapon/loot replays. Luminance is already installed. If a material is unreadable or visually weak, adjust that layer from the retained Git checkpoint without changing damage timing.

The separately merged Ghost Samurai update is retained: two-hit × four rhythm, grid-to-charge movement, late-lock dodges, longer last Phase2 charge hold and delayed wisps. All peers need the new protocol31 package. Its focused smoke is detailed in the [spec](encounters/ghost-samurai/ENCOUNTER_SPEC.md#検証と試遊); retain the repaired body/line rendering and re-summon cleanup. Retained Doll flight/beam/reward checks live in their evidence; an unrelated full Raid replay is not required.
