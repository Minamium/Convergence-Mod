...
Development **0.2.56 / protocol 29** on main, incorporating contributor commit `c2a9694`. Version comes from [build.txt](../build.txt); wire compatibility from [EncounterProtocol](../Common/Networking/Protocol/EncounterProtocol.cs). Public First Severance names remain **不幸な人形劇 / The Unfortunate Doll Play** and **ラクリモーサ — 縛られた心 / Lacrimosa — The Bound Heart**; internal identifiers and its accepted behavior remain stable. The coordinator now publishes Idle after the terminal snapshot so Ghost Samurai can be re-summoned; its summon/lifecycle diagnostics are in the [feature spec](encounters/ghost-samurai/ENCOUNTER_SPEC.md#再召喚の診断ログ). Attack tuning and existing Raid presentation are unchanged.
...
Ghost Samurai: reload/restart all peers on **0.2.56 / protocol29**. With the existing bell, summon → defeat or let the Boss retreat → summon again in the same world. Check `CombatEnded` followed by `ClientIdle` and a second `SummonAccepted`; any rejection now has a diagnostic reason. No unchanged Build + Reload is needed. The new feature's [spec](encounters/ghost-samurai/ENCOUNTER_SPEC.md#検証と試遊) owns broader checks; First Severance historical checks are not a mandatory repeat.
