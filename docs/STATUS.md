...
Development **0.2.66 / protocol 31** on `feat/ghost-samurai-combo-pressure`, incorporating integrated main `c5a6348`. [build.txt](../build.txt) owns version; [EncounterProtocol](../Common/Networking/Protocol/EncounterProtocol.cs) owns compatibility. The Ghost Samurai attack adjustment is built only in an isolated feature profile; shared installation awaits main integration. The main-side **0.2.65 / protocol30** native Release installation is recorded below, not a claim about every contributor's PC.
