...
Reload Mods/restart tModLoader to load the installed package; **no additional Build + Reload compilation is needed** for unchanged source. All participants need the matching protocol33 package. User-owned focused acceptance: initial/Spread/Final thin forecasts and needle→amplification; randomized lattice reveal/fire seen in the same order by peers; full-width Stillness coverage, twin rotation, Core and top/bottom/flood safe gaps. Verify that no wide hit precedes the visible mantle, including delayed snapshots and Reduced Effects/UI107%/zoom. Luminance is already installed. Do not replay unrelated weapon/loot or stage-ending checks for this narrow follow-up.

Ghost Samurai 0.2.69 needs protocol33 on every peer. Its [focused smoke](encounters/ghost-samurai/ENCOUNTER_SPEC.md#検証と試遊) covers harmless rush art/preparation contact, late dash, 90-tick charges and fixed-center out/in/out/in steps. The shared installation is complete; reload the installed package. Gameplay remains user-owned and unverified.
