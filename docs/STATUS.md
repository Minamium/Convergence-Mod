...
Development **0.2.66 / protocol 30**, native Release package installed from `a2fc281` on integrated main. [build.txt](../build.txt) owns version; [EncounterProtocol](../Common/Networking/Protocol/EncounterProtocol.cs) owns compatibility. First Severance remains **不幸な人形劇 / The Unfortunate Doll Play**, Boss **ラクリモーサ — 縛られた心 / Lacrimosa — The Bound Heart**. The owner authorized a Raid-wide presentation refresh after the eight-cast prototype: all Raid beam paths, energy bullets, source pressure, Stack/Spread verdict accents, stage/sky and both endings now use the new material/staging layers. [Visual spec](encounters/first-severance/VISUAL_SPEC.md#luminance-raid-presentation) owns scope, rendering contracts and pinned research. Combat rules/geometry/ticks, sounds/BGM, accepted shell/rig/capture art, weapons/companion/rewards and Ghost Samurai behavior are unchanged. Visual quality is experimental pending playtest, not a claim of WoTM/WotG parity.
