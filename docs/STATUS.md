...
Development **0.2.69 / protocol 33** adds Ghost Samurai contact rushes and Phase3 circles; its isolated feature package is built, with integration/shared installation pending. The retained **0.2.68 / protocol32** Doll baseline has an integrated native Release package; [build evidence](evidence/2026-09-13-beam-ignition.json) identifies the installed artifact. [build.txt](../build.txt) owns version; [EncounterProtocol](../Common/Networking/Protocol/EncounterProtocol.cs) owns compatibility. First Severance remains **不幸な人形劇 / The Unfortunate Doll Play**, Boss **ラクリモーサ — 縛られた心 / Lacrimosa — The Bound Heart**. All avoidable Doll Raid beams now use thin-axis forecasts, fast pilot launch, brief narrow hold and smooth rapid amplification instead of colored forecast volumes. Lattice lines reveal/fire in a shared deterministic scattered order. [Beam ignition](encounters/first-severance/ENCOUNTER_SPEC.md#beam-ignition-and-lattice-order) owns the changed geometry/timing; [visual spec](encounters/first-severance/VISUAL_SPEC.md#beams-swords-and-forecast-readability) owns the longitudinal material and preserved four-color roles. This is not a cosmetic-only hitbox change: authority and fractional art use the same envelope, and older peers are rejected. HP, fixed damage, assignments, music/sounds, player circles, shell/rig/capture/stage/endings, weapons/companion/rewards and Ghost Samurai implementation are retained. Visual quality is experimental pending playtest, not a claim of WoTM/WotG parity.
...
Reload Mods/restart tModLoader to load the installed package; **no additional Build + Reload compilation is needed** for unchanged source. All participants need the matching protocol32 package. User-owned focused acceptance: initial/Spread/Final thin forecasts and needle→amplification; randomized lattice reveal/fire seen in the same order by peers; full-width Stillness coverage, twin rotation, Core and top/bottom/flood safe gaps. Verify that no wide hit precedes the visible mantle, including delayed snapshots and Reduced Effects/UI107%/zoom. Luminance is already installed. Do not replay unrelated weapon/loot or stage-ending checks for this narrow follow-up.

Ghost Samurai 0.2.69 needs protocol33 on every peer. Its [focused smoke](encounters/ghost-samurai/ENCOUNTER_SPEC.md#検証と試遊) covers harmless rush art/preparation contact, late dash, 90-tick charges and fixed-center out/in/out/in steps. Gameplay and shared installation are not claimed by the isolated build.
