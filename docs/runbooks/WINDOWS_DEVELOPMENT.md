...
Use the available Python 3 interpreter. Doctor is read-only and identifies the resolved checkout, branch, dirty state, SDK and installed targets. Build uses that source and normal tML packaging, preserving the previous package in ignored `.local/builds/<timestamp>`. Its JSON record contains the exact commit, dirty status, SHA256 source-file manifest, resolved environment, command/log, output hash and actual packaged admission check; a source change during the build fails attribution. `--tml` and `--save` are explicit overrides; `--release-candidate` selects Release configuration, **never disables solo**, and does not approve publication. GUI builds require no opt-in symbol; if they replace a package before publishing, run the [exact-package admission check](../RELEASE_PROCESS.md#pre-release-gate) again.
