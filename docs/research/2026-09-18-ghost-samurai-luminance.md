...
| `Core/Graphics/Automators`: ManagedRenderTarget / RenderTargetManager | Use them through MetaballType's owned mask target; do not allocate per-frame textures or dispose dependency-owned assets. |
...
- PrimitiveRenderer consumes world positions and performs screen conversion internally. The ribbon is supplied bounded world-space tips; smoothing is disabled so a curve cannot overshoot into unrelated space. Duplicate tips, teleports and separate swings split/reset trails. Its pinned index loop emits N-2 segments; the latest main's installed-library finding also applies here. Append one tangent-support point and remap completion/width to keep the visible endpoint attached to the sword, without extending the visible geometry.
...
Convergence independently combines four motion envelopes, two articulated arms, recorded blade-tip ribbons, small cosmetic overrun and the approved purple oni identity. [Asset attribution](../../Assets/ATTRIBUTION.md) owns the generated parts and original HLSL provenance. The final offline FNA D3D11 preview uses the distributable shader exports and captured production part transforms:84 bright/dark rig frames plus6 diagnostic material frames. Smoke/ribbon diagnostics use synthetic geometry, not a simulated game or Luminance target-composition session. The native FNA wrapper signature was inspected for hidden-window device preparation; no Terraria/game instance is created. These frames and exact-package CPU checks do not prove in-game rendering, multiplayer synchronization or FPS; those remain explicit owner checks in [evidence](../evidence/2026-09-18-ghost-samurai-rig.json).
