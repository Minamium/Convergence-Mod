...
- **Phase-II lattice:** fine intersecting corridors use shared clipped grid geometry. Serial-three-and-later ordinary volleys add all-player Boss-origin Core salvos. The third-volley Spread pockets omit those salvos and resolve Spread during the live grid. **There is no Phase-II Stack.**
- **Twin blades:** a large forecast/brace followed by rapid unsheathing and two accelerating revolutions. Full sword aura and shared rigid motion define danger; no decorative paired rails.
