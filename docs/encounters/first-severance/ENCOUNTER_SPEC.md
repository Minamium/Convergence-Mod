...
- **Phase-I Prism:** eight rapid aimed casts for every standing participant simultaneously. Lock forecasts with movement lead before release; the colored forward/reverse order is shared.
- **Energy pursuit / Stillness:** right/left semi-homing energy charges alternate with center-out tightly packed narrow curtain teeth. Charge approach tracks then locks; dash-through is the intended motion. Stillness locks a safe column and fires adjacent teeth after each full warning; there is no hidden “velocity must be zero” damage test.
...
All avoidable Doll Raid beams warn with a thin locked axis, launch a narrow pilot from the origin, briefly hold narrow, then rapidly ease to the final width. [BeamIgnition](../../../Content/Encounters/FirstSeverance/FirstSeveranceBeamIgnition.cs) owns the shared envelope: travel completes in 3 ticks; a 1.5px half-width pilot holds through 1.5 ticks, then quintic amplification completes at tick 7. Authority uses integer samples; clients draw the same function at fractional time. Zero-length launch cannot damage even at its source. Rotation's swept samples also use the growing geometry. Short casts retain their end deadlines; no full-width damage precedes amplification. Fixed damage, targets, final widths, Final color order and hit ledgers are unchanged. Spread verdict feedback, energy-body contact, crush and player circles are not new dodgeable beams.

[GridVolley](../../../Content/Encounters/FirstSeverance/FirstSeveranceGridVolley.cs) deterministically shuffles line indices from accepted serial/start/pattern, then distributes offsets over 8 ticks. Reveal and fire receive the **same** offset: every line retains its complete 60-tick warning and 20-tick active interval. Each line retires separately; the grid ends after the last line, before the unchanged next cadence. Core salvos keep their own original end tick. Existing snapshots reconstruct the order without extra fields/actors. [CurtainComb](../../../Content/Encounters/FirstSeverance/FirstSeveranceCurtainComb.cs) keeps center-out pairs and adds the ignition duration to each tooth's hold so a genuinely gapless interval remains. Existing category boundaries follow the derived sequence duration. Horizontal floods adopt the same fast amplification while retaining safe-strip assignments, holds and embedded mechanic deadlines. All peers require the matching protocol.

## Four-color Final prism score

Four complete forecasts appear in **red → blue → green → gold** order, all before any fires. They keep their locked coordinates through a reading window, then fire in the same order. Later forecasts remain visible during earlier shots; harmless residue is dim. No numeric labels or arrows. This uses the initial eight-shot PursuitPrism's final corridor width, live duration, shared ignition geometry, finite-rectangle collision, emission material, aperture and launch sound—not the retired thin lattice-tooth effect.
