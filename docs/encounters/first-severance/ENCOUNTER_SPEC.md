...
- **Vertical interdict:** the legacy `HalfField` state uses two staggered waves of irregular top/bottom purple magic jets in place of swords. One half has no safe gaps when fully extended; the other has narrow whole-body gaps. Wave two shifts those gaps to require movement. The later action seals the opposite half. Warning covers the full future corridor; live damage reaches only the shared advancing light front, and withdrawal is harmless. Geometry/timing and internal `ImpalingSwords` IDs remain unchanged.
