...
Standalone Spread in every phase and the embedded Lattice/RemoteClaws Spread windows all schedule the same eight-step P1 pursuit prism, targeting every standing participant per step. Server snapshots freeze each aim including its velocity lead; no client retargeting. [Spread barrage](../../../Content/Encounters/FirstSeverance/FirstSeveranceSpreadBarrage.cs) distributes the eight starts across the window after a6-tick opening, retains the full28-tick warning and12-tick active interval for every cast, and ends the last beam24ticks before Spread resolves. Short windows overlap cast forecasts rather than truncate the reaction period. Missed starts are skipped without catch-up bursts. Damage is the existing120fixed beam hit, capped per cast/member; Spread itself retains zero damage when separated and its existing overlap penalty. Independent beams can therefore hit during an otherwise successful Spread.

The [authority runtime](../../../Content/Encounters/FirstSeverance/FirstSeveranceSpreadBarrageRuntime.cs) owns the bounded cast list and hit ledger, cleared on window/action/Fight end; the existing combat runtime retains tick ordering and terminal decisions. [Protocol](../../NETWORK_ARCHITECTURE.md#current-development-protocol-v25) owns the descriptor. P2 pocket and P3 flood coexistence, three/four-player crossings and real latency remain playtest checks, not guarantees from timing arithmetic. HP, other attacks, recovery and weapon behavior are unchanged.
