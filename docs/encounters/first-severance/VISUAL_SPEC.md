...
- **Thin axis with sparse footprint glints → fast needle launch → momentary narrow hold → smooth rapid amplification → dark residue** is the shared grammar for every avoidable Doll Raid beam. The continuous roughly one-pixel spine stays thin. Sparse, softly twinkling particles occupy the accepted future damage width **before** the axis is clamped; unoccupied pixels remain transparent. Use world/ray units, stable seeds and the accepted cast clock, not screen/UI scale or draw-time randomness. **No colored forecast volume, opaque rectangle, paired edge rails, arrows or branch-filled slab.** This supersedes the previous full-area warning policy, not the accepted player Stack/Spread circles.
- Initial eight-cast, standalone-Spread pursuit, Final four-color score, lattice, Core jet, Stillness teeth, twin rotation and top/bottom/horizontal jets all use the same original Luminance-managed material. **Moving bright heads with stretched tails apply across the beam families, not only lattice.** Sustained jets carry overlapping fast/slow asymmetric energy currents inside their continuously readable hazard; the finite lattice packet additionally transports its actual head/tail geometry. Do not mistake the dark space between internal highlights for safe gaps or replace sustained mechanics with short projectiles. Red/blue/green/gold keep their assignment and order; cool highlights and violet depths align them with the purple magic jet. Weapons/Oni retain their own presentation.
- The shader receives **currently growing geometry**, not the eventual full hitbox. Launch, brief hold and amplification are shared with authority through [Beam ignition](ENCOUNTER_SPEC.md#beam-ignition-and-lattice-order). Fractional drawing must never invent a target or use a different growth curve. Full-width damage starts only once the visible mantle reaches that width. Retired light is clearly dim and harmless. A continuous low mantle makes the whole live corridor readable; brighter internal threads are not dodgeable gaps.
- The single Phase-II Core jet has one thin forecast and a connected glowing throat. The rotating metal sphere's crater remains: inward displacement, raised lip, concave wall normals, dark depth, pressure hold and resealing. No floating muzzle, circular seal, square-cut root or parallel forecast subdivision. A persistent flow clock prevents phase jumps at release.
- Lattice lines deploy in a readable stagger derived from the accepted descriptor, retaining each full warning. Live energy is a **finite flowing ribbon: bright rounded head → stretched turbulent body → long tapering tail**, not a corridor blinking on/off. Its dedicated `RibbonPass` shares the authority profile; seed and flow coordinates stay attached to the original track/moving packet, including sanctuary clipping. The head crosses first and the tail actually leaves; no static full-width cooling rectangle. [Encounter timing](ENCOUNTER_SPEC.md#beam-ignition-and-lattice-order) owns durations. Phase-I Stillness retains center-out teeth. Confined lattice/teeth/flood bands suppress outside corona so surviving gaps remain legible.
- Twin rotation braces on thin axes, emits purple jets and makes the existing two accelerating turns. The legacy `HalfField` top/bottom jets keep dense/sparse halves and shifted second wave. Horizontal floods retain their moving safe strip and embedded Stack/Spread holds. All three use the shared narrow launch/amplification; there are no sword sprites or metallic release sounds.
...
**Material:** separately advected bright/dark currents have sharp travelling heads, tapered tails and continuous wrap envelopes. Distorted filaments break up the highlights; broad white blobs are rejected. The mantle feathers continuously inward across a variable contour rather than ending in a hard side wall. Its outer limit remains the exact authority width, with no artificial collision shrinkage. Forecast/live/harmless residue remain separate. Source pressure and short release glare connect to the body's flow. Confined lattice/teeth/flood bands have no outside corona. Reduced Effects lowers sparse-glint density and peripheral intensity without changing the axis, future extent or live geometry.
