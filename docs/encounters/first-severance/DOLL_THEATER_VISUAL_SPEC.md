...
2026-09-12の修正：殻本体は元の高精細な `NullCantorShell` を維持。Phase IIIの遠隔腕だけ旧型の黒い腱・象牙装甲・長い鉤爪へ戻す。本体の人形腕は維持し、それが別の手を操る構図とする。鉤爪16枚／胸の拘束殻16枚の実際に形が異なる描き分けへ拡張し、既存の関節の遅れ・うねり・表面屈曲を二次動作として併用する。
...
| `RestraintFrames.png` | 960×1408、240×352の4×4。胸の拘束殻の開口・左右の装甲・垂れ布が変形する16枚。小さい顔／本体の人形腕／外側の収容殻を置換しない |
...
顔・衣装・球の材質・髪の塊・ひび・鉤爪開閉・胸の装甲の描き分けはtexture。傾斜、親子関節、片側の張力、服／髪の遅れ、微振動、殻のヒンジと残骸、取り込み、Finalの分裂はcode。今回は2部位各16枚＋NPC12枚であり、全身60枚の手描きアニメやフレーム間の形状補間を実装したとは言わない。使わない汎用アニメーションDSLは追加しない。
