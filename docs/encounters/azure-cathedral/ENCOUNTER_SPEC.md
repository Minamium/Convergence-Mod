...
| 2 — Glacial crossfire | Diagonal arrival and braking hold; all segments announce thin target lines, then fire limited-homing frozen-energy missiles | Glass rain with two adjacent missing columns per five; a fixed-origin sweeping cyan sword beam |
...
| 5 — White-night deluge | Mirrored diagonal braking and segment volley | Offset rain and second sword-beam sweep |
...
Actors retain independent native HP scaled in `AzureRules.Life`, frozen at Ready. **Only the worm head accepts weapon hits** in this iteration; body/tail are not extra damage multipliers or targetable HP pools, including in Fury. The normal boss bar shows Liora + Vitrion, then only Vitrion in Fury. No bespoke drop or companion was requested.
...
2. **Devouring:** when both gates are satisfied, clear every owned hazard and protect both actors. Hide HUD for this bounded scene, frame the center, let Liora raise her sword again. Vitrion rushes in, brakes immediately before its visible mouth reaches her, and completes the final distance in slow motion. A single brief monochrome silhouette beat marks consumption; no gore or repeated fullscreen strobing. No local client decides the transition.
3. **Fury:** once the scene completes, refill the worm to100% exactly once. Liora no longer renders or attacks; her hidden native actor retains the exact-Fight projection. Faster long charges replace Liora's former covenant/judgment windows; diagonal segment volleys remain. Old-phase projectiles cannot become damaging again after the refill.
...
- Luminance: original PortalBeam/RaidEnergy warning, dust, corona and mouth passes retinted cyan; `AzureGlass` animates mirrored glass, luminous missiles/crystals, reflected-water cathedral, ice prisons/jaws, rifts and actual player/gather countdown circles. Layered advected frost wisps envelop selected body joints and the field floor; melting warps/fades glass with moving noise and a bright dissolution edge. No ornamental Boss UI circles. Dependency noise stays runtime-referenced, never vendored. Bounded reusable draw buffers and scoped GPU state; no server texture/audio loading.
...
Next owner smoke: preparation/Ready/entrance without black-mask/BGM gaps; head-only hits and20% floor; both orders of the two gates; sword raise → fast/slow consumption silhouette → full-HP Fury; lethal Fury → center rush/melting/faded music → re-invocation. Check long-body turns, segment-volley dodgeability and Liora's sweep, then2+ matching peers,107% UI/zoom and Reduced Effects. [Current evidence](../../evidence/2026-09-20-azure-devouring.json) distinguishes the old successful playtest, automated/offline checks and unrun new acceptance. No release certification is implied.
