...
**Rights boundary:** author description allows BGM/personal use, requires credit for secondary creative publication, retains kuku copyright and prohibits uncreative BGM-only/endurance content. It does not explicitly grant standalone asset redistribution. Consequently the locally approved game-facing OGG is gitignored, not published as a Git asset or release. Public Mod distribution of this new score needs author/source confirmation. A source-only build missing this recording keeps the other encounters available and rejects this summon with `crimson.licensed_music_not_installed`; it must not start a silently broken rhythm fight.
