...
One basic forecast crosses **from one field boundary to the opposite boundary** through the predicted position, never starting at the Boss. Doll's actual managed `PortalForecastPass`, sparse `ForecastDustPass` and red/magenta `PortalBeam` remain the foundation. A jet travels down the line and expands smoothly; shared geometry owns visible width/reach and native collision. Zero-width ignition and residue are harmless. Act II alternate phrases replace the four basic releases with white hairline space-tear warnings and fast red cuts. Act III alternate phrases use **four full-field vertical/horizontal cut lattices**, offset each beat; all other phrases retain aimed beams, and every ActI–III phrase closes with crossflow. `CrimsonSpatialCuts` owns lane spacing, offset, fast extension and short live duration; there is no client RNG or hidden collision drift. Each lattice has complete player-sized corridors and one full beat of warning.
...
The four Choir arms also appear in the Final ensemble; the complete body appears in Act III and the sacrificial return. Native actors, warning/live geometry, damage, phase timing and network state are unchanged. Vespera remains56px and point-sampled at her fixed native center. `ScarletSorcery` supplies original engraved red/black seals, white-to-red spatial cuts and black-flow material through Luminance-managed shaders. Client effects never change native targets.
