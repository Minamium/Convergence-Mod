...
ユーザー提供の「骸骨の鬼武者 × 青白い幽霊 × 二刀流」の画像と文章を基にした、別ボスの開発用実装。**Ghost Samuraiは開発中**で、Doll Raidの初期プロトタイプ完成とは別の進捗である。現在の実装・検証状態と既知問題は [STATUS](../../STATUS.md) を参照する。Doll Raidの戦闘・素材は変更しない。
...
- 最初の48tick（0.8秒）で現在位置から対象の左右320px付近へ減速移動する。左右のフィールド端からも320pxを確保する。対象の足元からフィールド内の最初の地形／足場をサーバーが読み、足場の高さに構える。床がない空中フィールドでは下端を基準面にする。地形は変更しない。
- 接近元・構え位置・衝撃波の高さを全員へ送る。構えに入った後の108tick（1.8秒）が溜め時間。最初の24tickだけ左右の向きを追い、固定後84tick（1.4秒）はボスの位置と向きを保つ。両刀を背中側に引く専用モーション、刀の淡い金色、視線方向の短い印と固定音で読む。安全地帯の印は描かない。
- 攻撃開始から156tickに正面半円を発動、168tickで終了する。半径2800pxでフィールドの正面全域を覆い、ボス中心より後ろの全身矩形には当たらない。実際の全身が前後の境界をまたぐ間は被弾し得る。背後にはフィールド端でも十分な幅が残る。
- 斬撃が終わった168tickに左右2個の衝撃波予告を同時生成。24tick（0.4秒）の無害な猶予を置き、192tickから左右へ14px/tickで進む。各波の判定は80×48pxで基準面より上側48px。描かれる縁と進む本体は同じ幾何を使い、基準面から60px跳べば全身が外れる。
- 左右へ1個ずつの波は、各フィールド端を抜けて消える。最後の波終了後18tickで通常選択へ戻る。波の発行イベントは1回だけで、背後で停止したプレイヤーにも届く。非常時でも600tick上限で自分の残存攻撃を解除して終了する。
- 衝撃波は捕捉した高さの水平面を走る。起伏に沿う地形改変・追従や、飛行禁止・ジャンプ強制は行わない。すでに十分高く飛んでいるプレイヤーはそのまま波を避けられる。平坦な足場で「接近→背後→ジャンプ」を最初に試遊する。
