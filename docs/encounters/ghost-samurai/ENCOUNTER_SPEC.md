...
根元はnativeの手に沿い、構えで照準と逆へ4px、解放の18Fで前へ8px、終端で0pxへ戻る。プレイヤー本体を押し出さない。腕は刀と同じ方向を向く最寄りのnative伸展段階を選ぶ。前進分と無害な接続には鍔の内側の差を許すが、表示とサーバーの刃・根元は同一曲線。溜め中の空振りダメージを避けるため、旧10–21Fの仮判定を解放からの`[14,23)`へ移す（14–18Fの加速、18–22Fの本斬撃、終点直後1F）。威力8400・3段目2.2倍、総26F、刃長560px、判定幅34px、各敵1ヒット、残心・Wraith Fire・ドロップは維持する。実効DPS／当てやすさが同一とは主張しない。対応版同士で遊ぶ。

`OboroFinisherArt`は同じ時計から、溜め中は紫の鬼火を刀へ収束、解放時は短い白紫の刀光とずれた刀身残像、振り抜き後は月弧の残光を描く。3段目の軌道強度は1.3、最大14tickで自然に消える。既存の刀・鬼火素材を使い、追加ダメージ・画面全体の閃光・新しい画像は作らない。Reduced Effectsは鬼火数・軌道層を減らす。振り音は18Fに各斬撃1回。

調整点：`PullEnd / HoldEnd / AccelerationEnd / CutEnd`が区間、`PeakSpeed / ExitSpeed`が重さと緩急、`AccelerationAngle / FollowAngle`が中間・終端姿勢、`PullDistance`と`OboroComboSettings.For(2).ForwardDistance`が引き／前進量。角度・基本26F・ヒット区間は定義表、残光は`OboroArt`、溜めの凝縮と解放光は`OboroFinisherArt`。3段目の掃引は曲線を最大64分割し、最高速度装備でも34pxの刃幅内で隣り合うサンプルを重ねる。
