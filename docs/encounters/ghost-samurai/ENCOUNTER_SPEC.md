...
### 朧のHeld Projectile骨組み — 2026-09-19

今回の範囲は構造の準備まで。既存の`Convergence/Oboro`、性能、振りの曲線、紫の残像を維持し、左クリック受理後に`OboroHeldProj`を1体生成する。コンボの1→2→3→1、命中、残心は従来どおりサーバー／SPが管理する。Projectileは受信した`comboIndex`・`timer`・`attackDuration`から毎tick位置と回転を更新し、クライアント用の`OboroHeldVisuals`が腕と刀を同じ姿勢で描く。native useStyleの刀表示と二重ダメージは無効。持ち替え・死亡・退出では削除する。

`Content/Items/Oboro/OboroComboSettings.cs`の`For(0/1/2)`が各段の調整場所。`BaseTicks`は基準フレーム数、`HitStart/HitEnd`は命中可能な時間の割合、`StartAngle/WindupAngle/CutEndAngle/ReturnAngle`は構えから戻しまでの角度、`Direction`は振り方向、`HoldOffset`は狙い方向への刀の根元の移動量（初期0px、プレイヤー移動ではない）。既存のSmoothStep/Hermite曲線は`OboroRules.Offset`に残し、判定と表示で共有する。

次のモーション制作では、今回指定された「素早い斬り上げ→鋭い返し→重いフィニッシュ」に角度・加速・つなぎを調整する。今回の骨組みではこの新軌道、踏み込み、新しいLuminance材質や腕同期の細部は完成扱いにしない。
