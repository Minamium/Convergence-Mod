...
振り始め→加速→鋭い振り抜き→余韻をつなぐ。9月19日のデータ設計で時間・角度・仮の補間を下記へ更新した。ダメージ判定と霊刃は同じ曲線を使用し、実体刀身と腕の表現は下記の9月21日仕様に従う。基礎威力・射程・判定幅・各敵1ヒット・ドロップ・レア度は維持するが、攻撃時間が変わるためDPS同一は保証しない。

紫の刀身残像、薄紫の月光状の刃先軌道、淡い霧を、実際に振った場所に残す。残像は1段目8tick・2段目6tick・3段目10tickで減衰し、連撃の切れ目でも自然に消える。残像自体に判定はない。プレイヤーごと最大16姿勢を記録し、別の斬撃同士の軌道は接続しない。持ち替え・死亡・退出・接続世代変更・大きな位置飛び・ワールド終了で記録を消去する。攻撃を止めた場合は10tickで通常の持ち姿へ戻す。Reduced Effectsでは残像と軌道の描画量を減らし、実体の刀と判定は同じまま。

### 朧の刀と霊刃の分離 — 2026-09-21

0.3.34の試遊で「刀を振っている感じがしない」と指摘されたため、実体のPNGを560pxへ拡大する表現を廃止する。実体は常時132px、金の鍔と紫の刀身を保持。`OboroSwordMotion.Wrist`で構えに手首を引き、腕が先行してから刀が追い越し、終端で手首を戻す。手首補正は1段目+0.32→-0.24rad、2段目-0.32→+0.26rad、3段目+0.42→-0.30rad。実体の柄はこの腕角度のnative handに接続し、左右反転する。サーバーの根元・角度・18/16/26F・ヒット区間・560px射程は変更しない。実体の手首表示から命中を決めない。

`OboroSlashMaterial`は受理した刀軸の履歴を三日月形のメッシュへ変換し、Luminance ManagedShader `OboroMoonArc`で白紫の鋭い外縁、流れる妖気、裂けて薄れる内側を描く。刀身の拡大は行わず、判定中だけ切っ先から560px先まで細い霊刃を表示する。残光の外縁も既存の判定半径を越えない。三日月の厚みは1/2/3段目140/95/190pxを上限とし、先端へ絞り、尾は8/6/10tickで消す。残光自体は無害。実体の残像は最大2枚（Reducedでは1枚）、最大16履歴、1区間48分割。Reducedでも現在の霊刃の長さを削らず、分割・模様・鬼火を減らす。ManagedShaderは依存側が所有し、描画状態は既存のWorldGraphicsScopeで復元する。

調整点は`OboroSwingPresentation.SwordLength`（実体サイズ）、`OboroSwordMotion.Wrist`（手首の溜め／返し）、`OboroSlashMaterial`（三日月の厚みと先端の絞り）、`OboroMoonArc.fx`（発光・妖気）。既存の角度曲線とLuminanceの無害な段間接続を維持する。Murasamaの瞬間的な鋭い三日月と、バージルの腕から切っ先へつながる振りを観察して独自に構成する。素材・コード・攻撃倍率は転載しない。実ゲームの腕／防具表示、連打感と主観的な見栄えはユーザーの次回試遊で確認する。

### 朧のHeld Projectileと3段コンボ定義 — 2026-09-19

既存の`Convergence/Oboro`と紫の残像を維持し、左クリック受理後に`OboroHeldProj`を1体生成する。コンボの1→2→3→1、命中、残心はサーバー／SPが管理する。Projectileは受信した`comboIndex`・`timer`・`attackDuration`から毎tick位置と回転を更新し、クライアント用の`OboroHeldVisuals`が腕・手首・刀を連動させて描く。native useStyleの刀表示と二重ダメージは無効。持ち替え・死亡・退出では削除する。
...
| 3 重い斬撃 | 26 | +150 | +170 | -70 | 14 / 23 | 最大8px |

ヒット区間と前進量は未指定部分の仮値。前進量は照準方向への刀の根元の追加移動量であり、プレイヤー移動ではない。攻撃速度補正は従来どおり段の開始時に取得し、段の途中では総時間を変更しない。ヒット区間もその総時間に比例して補正する。3段すべての角度と動的な持ち位置は下記の専用処理を使い、判定と表示で共有する。3段目の前進量は時間で変わる最大値。
...
### 朧の3段目フィニッシュ — 2026-09-20

`OboroThirdSwingMotion`が26Fを担当する。0–8Fで+150→+170度へ引き、8–14Fは角度と持ち位置を固定、14–18Fで+60度まで急加速、18–22Fで-70度へ斬り抜き、22–26Fで-95度まで余韻を残して減速する。接点速度は0／0／0／-72／-8／0度/F。全体を遅くせず、静止と最速の解放を分ける。26F終端を無理に1段目の+110度へ戻さず、次段の0–4Fの無害なLuminance Cubic InOut接続で受け継ぐ。

根元はnativeの手に沿い、構えで照準と逆へ4px、解放の18Fで前へ8px、終端で0pxへ戻る。プレイヤー本体を押し出さない。この根元は霊刃と判定で共有する。実体の柄と腕は9月21日仕様の手首姿勢で接続する。溜め中の空振りダメージを避けるため、旧10–21Fの仮判定を解放からの`[14,23)`へ移す（14–18Fの加速、18–22Fの本斬撃、終点直後1F）。威力8400・3段目2.2倍、総26F、刃長560px、判定幅34px、各敵1ヒット、残心・Wraith Fire・ドロップは維持する。実効DPS／当てやすさが同一とは主張しない。対応版同士で遊ぶ。

`OboroFinisherArt`は同じ時計から、溜め中は紫の鬼火を刀へ収束、解放時は短い白紫の刀光とずれた刀身残像、振り抜き後は月弧の残光を描く。3段目の月弧は上記の新しい材料を使い、最大10tickで自然に消える。既存の刀・鬼火素材を使い、追加ダメージ・画面全体の閃光・新しい画像は作らない。Reduced Effectsは鬼火数・軌道分割と材料の模様を減らす。振り音は18Fに各斬撃1回。

調整点：`PullEnd / HoldEnd / AccelerationEnd / CutEnd`が区間、`PeakSpeed / ExitSpeed`が重さと緩急、`AccelerationAngle / FollowAngle`が中間・終端姿勢、`PullDistance`と`OboroComboSettings.For(2).ForwardDistance`が引き／前進量。角度・基本26F・ヒット区間は定義表、残光は`OboroSlashMaterial`、溜めの凝縮と解放光は`OboroFinisherArt`。3段目の掃引は曲線を最大64分割し、最高速度装備でも34pxの刃幅内で隣り合うサンプルを重ねる。

今後の仕上げ目標は、ユーザー提供のMurasama動画の一太刀ごとの鋭い発光と薄く残る軌道、バージル動画の構え・鋭い解放・次姿勢まで連続した重心／刀の動き。朧の紫、妖刀、朧月の意匠へ独自に翻案する。動画の攻撃倍率、突進、追加攻撃、素材やコードを移植する指示ではない。[観察範囲](../../research/2026-09-16-oboro.md#supplied-motion-clips--2026-09-20)を参照。
...
`HandWeight` は0–3Fで手に接続、3–14Fはnativeの手に一致、14–16Fで次段の既存持ち位置へ接続する。霊刃とサーバー掃引判定は同じ曲線と根元を使う。実体の柄と腕は上記9月21日仕様を使う。急減速区間で判定の隙間を作らないよう、2段目だけ掃引サンプルの密度を上げる。1更新あたり最大64分割の既存上限は維持する。段頭の照準変更だけは既存のLuminance Cubic InOutで無害な予備区間中に接続する。残像は2段目のみ細く、最大6tickで減衰。振り音は6Fを基準に一度鳴らす。総時間16F、従来のヒット区間3–12F、威力、射程、ドロップ、残心、Wraith Fireを維持する。霊刃の根元には既存の補間を残し、実体の柄はnativeの手へ接続する。

調整点は専用クラスの区間終了F・中間角度・接点速度と`HandWeight`、基本角度は`OboroComboSettings.For(1)`。`OboroSwingPresentation.ReturnCutFadeTicks`が残像時間、`OboroSlashMaterial`の2段目widthが軌道の細さを担当する。速度補正は全区間を同じ比率で伸縮する。
...
`OboroFirstSwingMotion.HandWeight` と `OboroHandAnchor.Capture` が根元をプレイヤー中心からnativeの手へ移す。4–16Fは手に一致し、初めと最後は既存の持ち位置へSmoothStepで接続する。構えで後ろへ引いた手が斬撃で前へ出るが、プレイヤー本体は動かさない。これらは霊刃の根元を定める。実体の柄は9月21日仕様の手首姿勢へ接続する。サーバー判定は同じ回転と動く根元を掃引する。
