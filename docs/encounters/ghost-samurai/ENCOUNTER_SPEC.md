...
- 左クリック長押しで、上段からの袈裟斬り→小さな切り返し→大きく引く重い振り抜き。基礎攻撃力8400、威力1/1/2.2倍。刃の射程560px、太さ34px、地形越しの命中なし。True Meleeで各段・敵の共有HP本体ごと1ヒット。押している間は3段をループし、離したら現在の段を完了して停止する。次の押下は1段目から。持ち替え・制御不能・死亡・退出は直ちに中止する。各段の時間・角度は下記の調整表を正とする。
...
**9月26日の「武器修正案」を優先し、以下は0.3.47の現行値へ更新した。** 上段から斬り下ろし、小さく上へ返し、長い溜めから大きく斬り下ろす。1周を60Fから70Fへ約17%延ばし、振り抜く瞬間の鋭さを残す。威力・射程・判定幅・各敵1ヒット・ドロップ・レア度・残心・Wraith Fireは維持するが、時間と軌道が変わるためDPS同一は保証しない。攻撃の時計、霊刃の軌道とサーバーの判定は同じ定義を読む。通信形式とIDは維持し、異なる軌道の組合せを拒否するためprotocol71で統一する。

刀本体・刀身残像・腕の操作は**受理された攻撃モーション中だけ**。持っただけの待機時、受信前、攻撃終了後には表示しない。入力を離すと現在段を振り抜いて消え、次の押下は1段目から出現する。旧10tickの待機姿勢への復帰と待機用の代替描画を廃止する。インベントリ・地面のドロップ表示は維持。

紫の霊刃の残光だけは実際に振った位置から自然に消える。減衰は1/2/3段目8/6/10tick、最大16姿勢、別の斬撃同士は接続しない。残光に判定はない。持ち替え・死亡・退出・接続世代変更・大きな位置飛び・ワールド終了で記録を消去する。遅延した同一攻撃の時計で、消えた刀を再表示しない。

### 朧の刀と霊刃の分離 — 2026-09-21

実体は132pxの金の鍔・紫の刀身を維持。`OboroSwordMotion.Wrist`で構えに手首を引き、腕が先行してから切っ先が追い越し、終端で手首を戻す。手首補正は1段目-0.32→+0.24rad、2段目+0.26→-0.20rad、3段目-0.42→+0.30rad。柄はこの腕角度のnative handへ毎tick接続し、左右反転する。実体刀身を560pxへ拡大せず、その先は同じ刀軸の霊刃として描く。実体の手首表示から命中を決めない。

`OboroSlashMaterial`は受理した刀軸の履歴を三日月形のメッシュへ変換し、Luminance ManagedShader `OboroMoonArc`で白紫の鋭い外縁、流れる妖気、裂けて薄れる内側を描く。判定中だけ560px先まで細い霊刃を表示し、残光の外縁も判定半径を越えない。三日月の厚み上限は1/2/3段目140/70/210px。短い返しと大きな締めを区別する。実体の残像は攻撃中のみ最大2枚（Reducedでは1枚）、1区間48分割。Reducedでも現在の霊刃の長さを削らず、分割・模様・鬼火を減らす。ManagedShaderは依存側が所有し、描画状態はWorldGraphicsScopeで復元する。

調整点は`SwordLength`（実体サイズ）、`OboroSwordMotion.Wrist`（手首）、`OboroSlashMaterial`（三日月の厚み／先端）、`OboroMoonArc.fx`（発光／妖気）。素材や第三者コードを追加転載せず、既存の承認済み刀・鬼火と独自の動的材料を使用する。

### 朧のHeld Projectileと3段コンボ定義 — 2026-09-19

`Convergence/Oboro`のIDを維持し、左クリック受理後に無害な`OboroHeldProj`を1体生成する。コンボ1→2→3→1、命中、残心はサーバー／SPが管理する。Projectileは受信した`comboIndex`・`timer`・`attackDuration`から毎tick位置と回転を更新し、クライアントの`OboroHeldVisuals`が腕・手首・刀を描く。native useStyleの刀表示と二重ダメージは無効。待機中のholdoutは不可視。持ち替え・死亡・退出では削除する。

基本値は`Content/Items/Oboro/OboroComboSettings.cs`の`For(0/1/2)`で調整する。右向き・照準方向0度、画面下向きが正、Fは速度補正前、1tick=1F。ヒット区間は0始まりの `[HitStartFrame, HitEndFrame)`。

| 段 | TotalFrames | StartDegrees | WindupDegrees | EndDegrees | HitStartFrame / HitEndFrame | ForwardDistance |
|---|---:|---:|---:|---:|---:|---:|
| 1 袈裟斬り | 22 | -65 | -100 | +65 | 5 / 15 | 0px |
| 2 小さな切り返し | 18 | +90 | +102 | -40 | 3 / 12 | 0px |
| 3 大きな振り抜き | 30 | -65 | -130 | +90 | 16 / 26 | 最大8px |

前進量は霊刃の根元を照準方向へ動かす距離で、プレイヤーを移動させない。攻撃速度は各段の開始時に取得し、時計・ヒット区間・全動作を同じ割合で補正する。段の途中では総時間を変えない。3段すべての角度と動的な根元は下記の専用処理を使い、判定と霊刃で共有する。

`OboroTiming`が1tickずつ進め、段の終了時に`Step=(Step+1)%3`・`Age=0`として次段へ移る。照準は段開始時に採用し、進行中に入力で刃を回さない。入力更新が途絶えた場合も現在段を完了して停止する。[ADR-0027](../../adr/0027-oboro-authoritative-weapon.md)の所有権・通信・後始末を維持する。

### 朧の3段目フィニッシュ — 2026-09-20

9月26日案に合わせた`OboroThirdSwingMotion`の現行30F：0–8Fで-65→-130度へ大きく引き、8–16Fは角度と手首を固定。16–20Fで-30度まで急加速し、20–25Fで+90度へ振り抜き、25–30Fで+110度まで減速する。接点速度は0／0／0／+55／+7／0度/F。次の1段目は無害な0–5FにLuminance Cubic InOutで前の姿勢からつなぐ。

霊刃の根元は構えで照準と逆へ4px、20Fで前へ8px、終端で0pxに戻す。判定は溜めが終わる`[16,26)`のみ。実体の柄は手首姿勢のnative handを追う。`OboroFinisherArt`は溜め中に鬼火を刀へ収束し、解放時に短い白紫の刀光、振り抜きに大きい月弧を描く。振り音は20Fに一度。画面全体の閃光や追加ダメージは導入しない。

調整点は`PullEnd / HoldEnd / AccelerationEnd / CutEnd`、`PeakSpeed / ExitSpeed`、`AccelerationAngle / FollowAngle`、`PullDistance`。最大64分割の掃引で急加速と動く根元を追い、最高速度でも隣り合う判定が重なることを検証する。

### 朧の2段目モーション — 2026-09-20

現行`OboroSecondSwingMotion`は18F。0–3Fで前段終端+90→+102度へ小さく引き、3–6Fで+60度まで加速、6–11Fで-40度へ上返し、11–15Fで-55度へ減速、15–18Fで次段の-65度へ接続する。接点速度0／0／-33／-4／-2／0度/F。斬撃弧142度は1段目165度より小さく、加速区間も短い。

`HandWeight`は0–3Fで手に接続、3–15Fはnativeの手に一致、15–18Fで次段の根元へ接続する。霊刃と判定は同じ曲線。Luminanceの照準接続は無害な予備区間だけで完了する。振り音は6F、残光は6tick。調整点は専用クラスの区間・中間角度・速度、基本角度は`OboroComboSettings.For(1)`。

### 朧の1段目モーション — 2026-09-20

現行`OboroFirstSwingMotion`は22F。0–5Fで-65→-100度へ上段に引き、5–9Fで-25度まで加速、9–14Fで+65度へ袈裟斬り、14–19Fで+85度まで減速、19–22Fで次段の+90度へ接続する。各区間は接点速度を共有するHermite補間で、0／0／+34／+4／+1／0度/F。単純な等速回転にせず、上で止めてから前方下へ振る。

`HandWeight`と`OboroHandAnchor.Capture`が霊刃の根元をnativeの手へ接続する。5–19Fは手に一致し、前後は中心へSmoothStepで接続する。実体の柄は手首姿勢へ接続し、プレイヤーは押し出さない。振り音は9F、残光は8tick。左右反転でも刀・腕・判定の向きを揃える。

最終的な参考は、提供Murasama動画の攻撃中だけの刀と鋭い月弧、バージル動画の構え→解放→次姿勢の連続性。[観察範囲](../../research/2026-09-16-oboro.md#supplied-motion-clips--2026-09-20)を再利用し、第三者の倍率・突進・素材・コードは取り込まない。実ゲームの腕／防具、入力感と主観的な見栄えはユーザー試遊で確認する。
