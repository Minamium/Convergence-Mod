...
ヒット区間と前進量は未指定部分の仮値。前進量は照準方向への刀の根元の移動量であり、プレイヤー移動ではない。攻撃速度補正は従来どおり段の開始時に取得し、段の途中では総時間を変更しない。ヒット区間もその総時間に比例して補正する。角度は `OboroRules.Offset` の構え→斬撃→次段の構えという3区間のSmoothStepで仮につなぎ、判定と表示で共有する。

`OboroTiming`が1tickずつ進め、段の終了時に `Step=(Step+1)%3`・`Age=0` として次段へ移る。押下中の照準は次段の開始で採用し、進行中の刃を入力で回さない。入力更新の途絶はサーバー側の期限で検出し、現在段を完了して止める。通信の仕組みは[ADR-0027](../../adr/0027-oboro-authoritative-weapon.md)を参照。今回の範囲はデータと進行であり、重さ・緩急・腕同期やLuminance材質の追加作り込みは次のモーション工程とする。
