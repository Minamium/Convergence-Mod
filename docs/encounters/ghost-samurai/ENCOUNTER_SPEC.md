...
- 左クリック長押しで、素早い斬り上げ→返し斬り→大きく引く重いフィニッシュ。基礎攻撃力8400、威力1/1/2.2倍。刃の射程560px、太さ34px、地形越しの命中なし。True Meleeで各段・敵の共有HP本体ごと1ヒット。押している間は3段をループし、離したら現在の段を完了して停止する。次の押下は1段目から。持ち替え・制御不能・死亡・退出は直ちに中止する。各段の時間・角度は下記の調整表を正とする。
...
振り始め→加速→鋭い振り抜き→余韻をつなぐ。9月19日のデータ設計で時間・角度・仮の補間を下記へ更新した。ダメージ判定と実体刀身は同じ曲線を使用する。基礎威力・射程・判定幅・各敵1ヒット・ドロップ・レア度は維持するが、攻撃時間が変わるためDPS同一は保証しない。

紫の刀身残像、薄紫の月光状の刃先軌道、淡い霧を、実際に振った場所に残す。残像は1・3段目で最大14tick、返し斬りの2段目で最大9tickで減衰し、連撃の切れ目でも自然に消える。残像自体に判定はない。プレイヤーごと最大16姿勢を記録し、別の斬撃同士の軌道は接続しない。持ち替え・死亡・退出・接続世代変更・大きな位置飛び・ワールド終了で記録を消去する。攻撃を止めた場合は10tickで通常の持ち姿へ戻す。Reduced Effectsでは残像と軌道の描画量を減らし、実体の刀と判定は同じまま。

### 朧のHeld Projectileと3段コンボ定義 — 2026-09-19

既存の`Convergence/Oboro`と紫の残像を維持し、左クリック受理後に`OboroHeldProj`を1体生成する。コンボの1→2→3→1、命中、残心はサーバー／SPが管理する。Projectileは受信した`comboIndex`・`timer`・`attackDuration`から毎tick位置と回転を更新し、クライアント用の`OboroHeldVisuals`が腕と刀を同じ姿勢で描く。native useStyleの刀表示と二重ダメージは無効。持ち替え・死亡・退出では削除する。

`Content/Items/Oboro/OboroComboSettings.cs`の`For(0/1/2)`だけで各段の基本値を調整する。角度は右向き・照準方向0度、Fは攻撃速度補正前、1tick=1F。ヒット区間は0始まりの `[HitStartFrame, HitEndFrame)` とする。

| 段 | TotalFrames | StartDegrees | WindupDegrees | EndDegrees | HitStartFrame / HitEndFrame | ForwardDistance |
|---|---:|---:|---:|---:|---:|---:|
| 1 斬り上げ | 18 | +110 | +135 | -35 | 4 / 13 | 0px |
| 2 返し斬り | 16 | -50 | -80 | +120 | 3 / 12 | 0px |
| 3 重い斬撃 | 26 | +150 | +170 | -70 | 10 / 21 | 0px |

ヒット区間と前進量は未指定部分の仮値。前進量は照準方向への刀の根元の追加移動量であり、プレイヤー移動ではない。攻撃速度補正は従来どおり段の開始時に取得し、段の途中では総時間を変更しない。ヒット区間もその総時間に比例して補正する。3段目は `OboroRules.Offset` の3区間のSmoothStepを維持する。1・2段目の角度と動的な持ち位置は下記の専用処理を使い、判定と表示で共有する。

`OboroTiming`が1tickずつ進め、段の終了時に `Step=(Step+1)%3`・`Age=0` として次段へ移る。押下中の照準は次段の開始で採用し、進行中の刃を入力で回さない。入力更新の途絶はサーバー側の期限で検出し、現在段を完了して止める。通信の仕組みは[ADR-0027](../../adr/0027-oboro-authoritative-weapon.md)を参照。

### 朧の2段目モーション — 2026-09-20

`OboroSecondSwingMotion.Angle` が16Fの返し斬りを担当する。角度は右向きの照準軸を基準とする。0–3Fは前段終端の-50度から-80度へ引き、上で減速する。3–6Fで-20度まで急加速、6–10Fで+120度へ振り下ろし、10–14Fは+143度まで減速、14–16Fで既存3段目の+150度へつなぐ。Hermite補間の接点速度は0、0、+42、+6、+2、0度/F。1段目とは回転方向が逆で、加速区間が短く、振り抜きの最高速度も大きい。

`HandWeight` は0–3Fで手に接続、3–14Fはnativeの手に一致、14–16Fで次段の既存持ち位置へ接続する。刀・腕・サーバー掃引判定は同じ曲線と根元を使う。急減速区間で判定の隙間を作らないよう、2段目だけ掃引サンプルの密度を上げる。1更新あたり最大64分割の既存上限は維持する。段頭の照準変更だけは既存のLuminance Cubic InOutで無害な予備区間中に接続する。残像は2段目のみ細く、最大9tickで減衰。振り音は6Fを基準に一度鳴らす。総時間16F、従来のヒット区間3–12F、威力、射程、ドロップ、残心、Wraith Fireを維持する。Nativeの腕の伸びは段階式のため、次段への無害な境目には最大約9.34pxの持ち位置差が残る。

調整点は専用クラスの区間終了F・中間角度・接点速度と`HandWeight`、基本角度は`OboroComboSettings.For(1)`。`OboroSwingPresentation.ReturnCutFadeTicks`が残像時間、`OboroArt`の2段目strengthが軌道の細さを担当する。速度補正は全区間を同じ比率で伸縮する。

### 朧の1段目モーション — 2026-09-20

`OboroFirstSwingMotion.Angle` が18Fの5区間を担当する。0–4Fは+110→+135度で静かに引き、4–8Fで+65度まで加速、8–12Fで-35度へ斬り上げ、12–16Fで-47度まで減速、16–18Fで2段目の-50度へ接続する。各区間は接点の速度を共有するHermite補間で、8F時点の速度は-35度/F、12Fで-5度/F、16Fで-1度/F、18Fで0。総時間・ヒット区間・威力は変更しない。

`OboroFirstSwingMotion.HandWeight` と `OboroHandAnchor.Capture` が根元をプレイヤー中心からnativeの手へ移す。4–16Fは手に一致し、初めと最後は既存の持ち位置へSmoothStepで接続する。構えで後ろへ引いた手が斬撃で前へ出るが、プレイヤー本体は動かさない。`Stretch` は刀と同方向を向く腕の伸びを選び、段の境目の小さな持ち位置差を抑える。サーバー判定は同じ回転と動く根元を掃引する。

Luminance Cubic InOutを初動の無害な長さ・照準接続に使用する。実刃の角度は共有曲線に一致させる。刀身と既存残像の左右反転を統一し、左向きでも1→2段目で刃の反りが反転しない。音は最速部の8Fに合わせる。2・3段目の曲線、時間、威力、残心やドロップは維持し、新素材や追加攻撃は導入しない。
