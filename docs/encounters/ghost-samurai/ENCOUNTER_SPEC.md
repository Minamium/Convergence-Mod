...
今回の調整のユーザー確認は同じ版を読み込んだ複数人で、攻撃後の待ち時間、四連斬撃の予告と4撃目、広い格子の隙間、剣撃回避後に拡散→追尾する鬼火と残弾量。特に通信遅延時・途中参加時の鬼火の表示と判定、格子中の処理負荷、フェーズ移行・退場後の残弾消去は実ゲームで確認する。実装備での回避しやすさは未測定。GUI・ゲーム・サーバーはユーザーが起動する。
