...
ヒット区間と前進量は未指定部分の仮値。前進量は照準方向への刀の根元の追加移動量であり、プレイヤー移動ではない。攻撃速度補正は従来どおり段の開始時に取得し、段の途中では総時間を変更しない。ヒット区間もその総時間に比例して補正する。2・3段目は `OboroRules.Offset` の3区間のSmoothStepを維持する。1段目の角度と動的な持ち位置は下記の専用処理を使い、判定と表示で共有する。
