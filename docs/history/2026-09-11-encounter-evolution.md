---
doc_id: history.encounter-evolution-0246
...
source_of_truth_for: []
aliases:
  - Encounter evolution through 0.2.46
related_code: []
related_docs:
  - project.status
  - handoff.windows
...
The cause was `ApplyRaidDamage`'s intentional direct HP/Down path bypassing native `OnHurt`. The historical compatibility bridge performed only the gauge side effect; replaying Hurt alongside direct subtraction would have duplicated damage. That bridge was removed when [ADR-0024](../adr/0024-native-raid-hurt-and-downed.md) replaced direct subtraction with native Hurt, rather than running both. [Network architecture](../NETWORK_ARCHITECTURE.md#preceding-development-protocol-v24) preserves the old delivery contract.
...
|---|---|---|
| I / Sealed | Existing opening Pylon → fixed Stack → Spread → full Core exposure; then four clockwise Stack sites, with Spread after each | 80% → six-second Unbound eclosion |
| II / Unbound | Grid block → Spread → rotating blade → Spread → grid block → Spread | 40% → five-second distant retreat |
...
The first full Core exposure is never interrupted by reaching its HP floor. After it, four sites proceed top → right → bottom → left around the arena center. Every site is a **stationary world coordinate for that action**, not a following player or moving damage disc. Current valid radius remains 112px; only that radius is circular, with directional chevrons and no ordinal label. Ellipse radii are 560px horizontally and 300px vertically. Final uses the same ellipse with eight 45-degree positions. Distant's two assemblies use the top and bottom sites.

Damage clamps at the next stage's floor, including lethal NPC hits; overkill is discarded. At a floor the aperture closes and HUD shows HP LOCK while the first score continues. Its completion releases a pending transition. If the floor was not reached on that cycle, another cycle begins; after one complete cycle, threshold arrival interrupts the current damage action on that same authority tick, without waiting for its deadline. Old lance/grid/Spread/score hazards do not fire on that transition tick and action cleanup cancels them. Due mechanic adjudication and recovery/terminal priority retain their existing order. Nothing can skip the first score or kill the Boss before Final. Logical HP zero is replicated; the temporary NPC's technical one HP is not a second health bar. Final is not counted as an ordinary numbered phase, so future IV/V can be inserted before it. The old eight-exposure hard cap is not applied to this staged score; Pylon Overload and recovery failure still apply.
...
The [Revive Specification](../encounters/first-severance/REVIVE_SPEC.md) is the only recovery-rule source: untimed Down, reusable instant kit, recipient lockout, no active Eliminated. The frozen roster does not shrink. General Terraria lethal events are not converted into Raid Down by this experiment.
...
The [pre-consolidation snapshot](2026-09-07-pre-consolidation.md) preserves superseded single-cycle timing and staged implementation history. It is not current tuning or an activation gate.
